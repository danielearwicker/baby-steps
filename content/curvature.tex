...
We should be clear about the thing we are trying to mimic as closely as we can. We have shown that it is possible to cover the surface of a sphere with a two dimensional coordinate system. It therefore follows that (excepting the awkwardness at the poles) we can think of the points on the surface as isomorphic with $\mathbb{R}^2$, the set of all pairs of real numbers. This is easily regarded as a vector space, of course, but what physical resemblance does it have to the curved surface we're trying to describe? The difference between two vectors in $\mathbb{R}^2$ is no guide to the distance between the two corresponding points on the surface. Not only is the curvature not captured, but the arbitrary choices made by the coordinate system will affect the results as well. What we really want is not merely a vector space but an \textit{inner product space}, that is, a vector space with an associated metric tensor, which accounts for the actual geometry of the surface in relation to whatever coordinate system we have wrapped over it.

The problem is that with a curved surface, the metric tensor may need to vary from place to place. Even if it doesn't vary, its description in terms of the coordinates almost certainly will need to vary. This is impossible with a single vector space. Only one metric tensor may be chosen for a given vector space.
...
Also on a curved surface, the tangent plane will likely be tilted differently between the two points, making it inherently difficult to say that any two vectors based on different origins are parallel. This is the essence of the problem facing us. In an affine space, there is only one vector space to map pairs of points to, and to that one vector space we can only associate one metric tensor to allow us to map vectors to physical distances. Curvature means that, although we can map pairs of points to vectors to at least some extent, we can't do so in a way that supports a consistent notion of physical distance everywhere on the surface.
...
We asked about the length of the basis vectors, and the tool we need to answer this question is an inner product (§\ref{inner-product}), as defined by the metric tensor. This will also gift us an isomorphism between vectors and covectors.

More precisely, as every point has its own independent vector space with its own basis vectors, we need a different metric tensor for each point: a metric tensor field. Indeed, this only makes sense because each point has its own separate vector space, because a vector space can only have one metric.

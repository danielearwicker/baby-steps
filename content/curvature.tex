...
Our intuitive picture of vectors as arrows is only going to mislead us now. An arrow has direction and length. Clearly at any point on the sphere we can choose a direction leading away from that point, and we know that the closest thing to a straight line on the surface of a sphere is a great circle, and we can measure distance around such a circle if we know the radius, which for a great circle is the same as the radius of the sphere itself. So what if we think of vectors as arcs of great circles? These curved arrows will have a direction and a length, and this suggests we can add them by placing them head to tail as usual, so the geodesic curves connecting points on the sphere correspond to elements of a vector space.

But this fails even the simplest requirement of a vector space. Vector addition must be commutative: $\vec{a} + \vec{b} = \vec{b} + \vec{a}$, and we usually picture this as a parallelogram giving us two equivalent routes between opposite corners. Suppose you stand on the equator facing north. You are going to make a journey by turning 90 degrees to the right and walking straight for 100 metres, then 90 degrees to the left and travelling 10,000 metres. That is, you will walk along the equator a short distance and then north along a curve of longitude for a much longer distance. You will therefore end up at a latitude that is exactly 10,000 metres from the equator. This is like adding two of our would-be vectors: the first leg is $\vec{a}$, the second is $\vec{b}$, and so the final destination is $\vec{a} + \vec{b}$. This is version 1 of the journey. 
...
We should be clear about the thing we are trying to mimic as closely as we can. We have shown that it is possible to cover the surface of a sphere with a two dimensional coordinate system. It therefore follows that (excepting the awkwardness at the poles) we can think of the points on the surface as isomorphic with a rectangle in $\mathbb{R}^2$, the set of all pairs of real numbers. This is easily regarded as a small patch of vector space, of course, but what physical resemblance does it have to the curved surface we're trying to describe? The difference between two vectors in $\mathbb{R}^2$ is no guide to the distance between the two corresponding points on the curved surface. Not only is the curvature not captured, but the arbitrary choices made by the coordinate system will affect the results as well. What we really want is not merely a vector space but an \textit{inner product space}, that is, a vector space with an associated metric, which accounts for the actual geometry of the surface in relation to whatever coordinate system we have wrapped over it.

The problem is that with a curved surface, the metric may need to vary from place to place. Even if it doesn't vary, its description in terms of the coordinates almost certainly will need to vary. This is impossible with a single vector space. Only one metric may be chosen for a given vector space.

\subsection{Affine spaces}

Even on a flat infinite surface there is no natural identification between the points in that surface and the elements of a vector space. The reason for this is that any such mapping requires us to nominate an origin, so that each point can be identified with the vector that reaches from the origin to that point. This means, obviously, that the origin is identified with the zero vector. But the location of the origin is arbitrary, something we introduce by making a choice. A different choice of origin will move the zero vector to a different location, so there is nothing unique about any given identification of points with vectors.

Also there is something ridiculous about hastily identifying points in a space with vectors, which can be seen more clearly if we consider a one-dimensional example: time is a one dimensional space of points that we can identify with the real numbers, and the real number line is a one-dimensional vector space. To make this identification, we need to nominate a point in time to be $t=0$ (the zero vector and origin of the vector space). The first thing we learn about vectors is that we can add them. What do we get if we add two points in time, $t_a + t_b$? The result is \textit{meaningless}. You might get a very large number if both these times are far from the arbitrary $t_0$, or $0$ if they are on either side of $t_0$, but there is no physical meaning to this result as it is determined entirely by where we chose to place the origin.

This is why programming languages with proper data types to represent times will refuse to define an addition operation on time values. However, they do support a subtraction or difference operation on an ordered pair of time values. The result of this operation is not the same kind of time value, but a different abstract type called something like duration, time span or time interval. These are vectors! They have a physical meaning that is independent of any origin. Indeed, by performing the subtraction we effectively cancel out the contribution of the origin.

So points in time are not vectors, but there is a \textit{difference} operation that maps pairs of points to vectors. An \textit{affine space} is the combination of three things:

\begin{itemize}
    \item a set of objects called points,
    \item a vector space, and
    \item a difference operation $d(A, B)$ that maps pairs of points to vectors
\end{itemize}

Also there is a rule that applies to the difference operation, called \textit{transitivity} for any three points $A$, $B$ and $C$:

$$
d(A, B) + d(B, C) = d(A, C)
$$

Two things follow very simply from this. If we set $A$, $B$ and $C$ to all be the same point $A$:
...
That is, all these things are vectors, and the zero vector is the only vector that can be added to itself to give itself, from which we conclude that the difference of a point with itself must be the zero vector. We can also set $A$ and $C$ to be the same point, but leave $B$ independent:
...
So the difference operation is \textit{antisymmetric}. The vector obtained from the difference of $A$ with $B$ is equal and opposite to that obtained from the difference of $B$ with $A$.

Nominating an origin is just like binding the first parameter of the difference operation to a specific point, by which we obtain a simple mapping of points to vectors. We can go further than this in the usual way and map the points to vectors in $\mathbb{R}^N$, by choosing $N$ basis vectors, which has the advantage of allowing us to perform arithmetic on the vectors, but we must remember that we can only obtain physically relatable results if we always begin by performing subtractions between vectors. Additions will only amplify the arbitrary choice we made when choosing the origin.

Two different observers may choose different origins, but as long as they choose the same basis vectors, their results in $\mathbb{R}^N$ will be easy to translate between. But note that we have made an interesting assumption here, that there is such a thing as "the same basis vector" at two different points.

Intuitively we know that on a flat surface this means that the first person's $\vec{e}_1$ is both parallel to, and the same length as, the second person's $\vec{e}_1$. But this implies we have a concept of norm (length) and angle, which we know only appears when we have an inner product.

Also on a curved surface, the tangent plane will likely be tilted differently between the two points, making it inherently difficult to say that any two vectors based on different origins are parallel. This is the essence of the problem facing us. In an affine space, there is only one vector space to map pairs of points to, and to that one vector space we can only associate one metric to allow us to map vectors to physical distances. Curvature means that, although we can map pairs of points to vectors to at least some extent, we can't do so in a way that supports a consistent notion of physical distance everywhere on the surface.

It seems for the moment we have to abandon any attempt to relate vectors at different locations to one another on a curved surface. But we can recover a form of this capability by stealth if we tread carefully. It just won't be an affine space.
...
If there is a smoothly varying scalar field in the neighbourhood of a point, there is \textit{unavoidably} a gradient vector at that (and every) point. Furthermore there are infinitely many possible scalar fields that could be overlaid on the surface, each causing a different gradient to exist at a given point. We say that there is an entirely separate, isolated vector space at each point called the \textit{tangent space} at $\mathcal{P}$, whose elements are the vector gradients that would be associated with every possible scalar field around that point. This is sometimes pictured as a flat plane touching the surface at the point, and thus tangent to it. It is a perfectly good intuitive picture, but it's important to remember that we are interested here in describing the curved surface without reference to anything "outside" it.

By tangent we really mean the idea in calculus that the derivative of a function gives us the linear approximation to the function around some location, the gradient of a tangent line. 

Indeed, if we imagine a curve that passes through a point, there will be a line tangent to the curve, and we can find the gradient of the scalar field along that line. By doing this, we have uncovered an ordinary function of one variable (position along the curve), of which we may take the derivative. This is the directional derivative of the scalar field at some point, and is just a scalar, but to define it we first had to choose a direction. A vector in the tangent space is a direction along which we could take the derivative of a scalar field over the actual space.

Even before we choose our coordinate fields $x^i$, a vector space already "exists" at every point merely because scalar fields are a theoretical possibility. When two particular scalar fields have been overlaid on the surface to provide every point in some neighbourhood with coordinates, this picks out basis vectors $\vec{e}_i$ at each point, pointing along the direction of motion due to an infinitesimal increase of the $i$th coordinate, and we can use these as the basis vectors within each point's tangent space.

Near to some area of interest these basis vectors are hopefully almost orthonormal, although further away, if the surface is curved, they will have to diverge more and more from orthonormality (either by a change in angles, or relative length, or both.) But in any case, they provide each point's vector space with a set of basis vectors, as we assume they are linearly independent (at least over some useful area of the surface) and can therefore be blended in a linear combination to generate every possible vector in that point's vector space. 

We can make all these concepts more concrete by imagining that every point on the surface of a sphere has a pair of tangent arrows sprouting from it: a red arrow (the first basis vector) and a blue arrow (the second). At some location they may happen to be perfectly orthonormal, but at neighbouring points they will gradually diverge from orthonormality. In some places they may even become colinear, or one of them may become the zero vector, either of which is a disaster, but this kind of problem is inescapable when trying to cover a whole sphere with one coordinate system. It doesn't matter; we just need there to be a point where the red arrow points in a different direction from the blue arrow, and for it to be surrounded by similar such points.
...
On the left, we are scaling the first basis vector by the factor $a$, the result being a vector at each point that is colinear with that basis vector. On the right we are finding the rate of change of some scalar field $a$ as we change only the first coordinate, holding any other coordinate(s) constant, the result being a scalar at each point. It may be that $a$ doesn't vary due to that coordinate so the result is always $0$. It may be that $a = x^1$ everywhere, so the result everywhere is $1$. But it is a scalar, not a vector.
...
We asked about the length of the basis vectors, and the tool we need to answer this question is an inner product (§\ref{inner-product}), as defined by the metric. This will also gift us an isomorphism between vectors and covectors.

More precisely, as every point has its own independent vector space with its own basis vectors, we need a different metric for each point: a metric field. Indeed, this only makes sense because each point has its own separate vector space, because a vector space can only have one metric.

But at each point, in that point's private metric space, it works as usual. Given a metric $g_{ij}$ at some point, we can convert the coordinates of any vector $\vec{v}$ chosen from that point's tangent space into the corresponding covector coordinates:
...
The metric will turn out to be the key to connecting our coordinate system to the geometry of physical space. A change of the $x^i$ coordinate by some amount induces a real physical displacement by some distance. The metric at the starting point gives us that distance, or rather the linear approximation of it at the starting point, which becomes less reliable the larger the change of the coordinate.

The nature of the curvature is described by the change in the metric's matrix representation as we move around the surface. If the metric is described by the identity matrix in one location, it will have to diverge from that description in neighbouring locations to account for the curvature. In the (latitude, longitude) coordinate system the longitude basis vectors get shorter toward the poles, and this has to be captured in the metric's representation in coordinates.
...
If we have functions $p^i(t)$ giving the two coordinates $i \in \left\{ 1, 2 \right\}$ of a moving object on the surface, we can obtain the coordinates of velocity vector $\vec{v}$ at time $t$ by taking the time derivative of the position coordinates:

$$
v^i(t) = \frac{d p^i(t)}{dt}
$$

This is an entirely valid vector, but only at the point $p^i(t)$. We could define the path as:

$$
p^1(t) = t \quad \quad p^2(t) = 0
$$

That is, the object is travelling along a curve of constant second coordinate, and its speed happens to be just so that the first coordinate tells you how many seconds have elapsed. Therefore at any point on the curve, the curve is aligned with the tangent vector given by the coordinates $(1, 0)$, which is just the first basis vector.
...
In keeping with the interpretation of $t$ as the elapsed time, and $x^i(t)$ as the coordinates of something moving along the curve, $v^i(t)$ would be the velocity at $t$. Equivalently it is the tangent vector to the curve at it passes through $x^i(t)$ at time $t$ (it is possible that the curve may visit the same spot at different times, and the velocity vector may be different each time.)
...
Here we're using the notation $g_{ij}(t)$ to remind ourselves that $g_{ij}$ is not a constant but is smoothly varying across our journey. Also note that we're using Einstein summation notation as usual, so the above is the sum over all pairs of $(i, j)$ index values.
...
But this independence does not apply to the basis vectors. In the standard global coordinate system, as we move north from the equator along a curve of constant longitude, only our latitude is changing. But the longitude basis vector associated with each point becomes ever smaller, because if we were to increase the longitude coordinate, the resulting displacement would be smaller the further north we'd reached. This is a relatively simple example because at least the basis vectors remain orthogonal and only change length; in general they could twist relative to each other as well.
...
Also, just like a tensor, we can raise and lower indices using the metric. So for example:
...
That is, the $k$th basis covector acts on the vector that is the derivative of the $i$th basis vector with respect to the $j$th coordinate. Having reduced these to a pseudo-tensor, we can raise and lower the indices with the metric as we saw before.

Meanwhile, the metric is just the inner product of all pairs of basis vectors:
...
The result of the metric applied to two vectors, i.e. the inner product, is a scalar, so we are taking the derivative of a scalar function.
...
$$
v^i(t) = \frac{d c^i(t)}{dt}
$$

Note that this is not the same as taking the derivative of a vector field: the coordinates of points are scalars, not vectors.
...
In other words, the covariant derivative may say that there is no change to the vector, even though the numeric representation of the vector is changing. Conversely, the velocity vector's coordinates might remain constant, such as when moving due East at constant speed along some curve of constant latitude other than the equator, and yet the covariant derivative may be non-zero. This is the effect of the extra Christoffel term in the definition of the derivative, but it has the effect of producing a result that is independent of the coordinate system, i.e. it always describes the same derivative vector, in terms of the chosen coordinate system.
...
Does this mean we've found some kind of "straight" path? Not at all. The vector field could have any configuration, independent of the geometry of the surface. The curve we are following may have to wind this way and that, as we feel our way through a messy vector field looking for "the same" vector right next to our current position.

But we can get as close as possible to a straight path, with reference to the geometry of the space. We are at last ready to say exactly what a geodesic is:
...
So we just need to figure out $D$, which has 8 combinations, but this is also simpler than it might appear. A partial derivative term will vanish if it's the derivative of something that is constant with respect to the denominator. Of all the metric components, $g_{\phi\phi} = \sin^2 \theta$ is the only variable and so could have a non-zero derivative --- but then, only if the bottom of the derivative is $\theta$. That is, we only care about the combinations where we have two $\phi$s and a $\theta$, and even then only one of the terms will be non-zero (the one that has the $\phi$s on the top and the $\theta$ on the bottom), and in all three of these cases the derivative will be (using the product rule and the fact that the derivative of $\sin$ is $\cos$):
...
The remaining index $k$ lets us choose which of the components of $\grad_{\phi} \vec{v}$ we are calculating. When $k=\theta$:
...
Fortunately we can use a simple symmetry argument to avoid all this! All that is required is to rotate the coordinate system (move the poles) so that our great circle is aligned with the new equator. Obviously this is cheating, but it works, and it underscores the geometric truth behind all this. A geodesic curve is a geodesic curve, regardless of our choice of coordinate system. The hard work we have to do to describe the behaviour of tangent vectors in terms of coordinates can obscure this underlying simplicity.

One subtle point is that our rule for a geodesic could be considered slightly too strict:
...
A parameter on a curve that, if increased at a constant rate, produces constant speed motion along the curve, is called an \textit{affine parameter}, the name coming from the fact that such a parameter $x$ can be wrapped in a function of the form $x \mapsto ax + b$ where $a$ and $b$ are arbitrary constants, an affine transformation, while preserving the constant speed of the motion.
...
\subsection{Non-coordinate basis vectors}

We've seen how beginning with a set of scalar fields $(x^1, x^2, \dots x^N)$, we can associate a unique combination of values $x^i$ with each point in our curved space (at least over some neighbourhood of it), so they serve as coordinates, and this setting is isomorphic to $\mathbb{R}^N$, but any vector operations performed in the "shadow" $\mathbb{R}^N$ will be ignorant of the geometry of the actual space.

By varying the $i$th coordinate $x^i$ but holding the others constant we can trace out a curve. Around a given point $\mathcal{P}$ through which that curve passes, there must be a line tangent to the curve. That line is a direction along which we can take the derivative of any scalar field at $\mathcal{P}$, with respect to the $x^i$ coordinate. For a scalar field (which can be thought of as a function of all the coordinates), this is just an ordinary partial derivative with no ambiguity about its definition, and so the operation of \textit{taking a derivative of a scalar field} serves as the source of basis vectors for the \textit{tangent space} at $\mathcal{P}$, each point having its own entirely separate tangent space. This means each point can have its own definition of a metric that relates vectors to distances and angles. The metric can vary smoothly across the space, so it is a \textit{metric field}, an example of a tensor field.

Given a metric we are able to define the covariant derivative, making it possible to compare vectors from different tangent spaces (at different points). A vector field is a function that assigns a vector to each point in the space. The covariant derivative of such a field along a curve will be zero if the vector is \textit{parallel transported} along that curve, which is to say that in a continuous smooth way it remains the same vector at each point along the curve, although:

\begin{itemize}
    \item its coordinates may be changing (though the coordinate system is not a reliable guide to the geometry of the space)
    \item if the curve is closed so it returns to the starting point, the vector may have different coordinates that it had originally at that point, which most certainly tells us something about the geometry of the space.
\end{itemize}



\subsection{Travelling around a closed loop}

The experience of a traveller on the globe, attempting to carry an arrow parallel to the surface on a journey around a closed path, while making sure to avoid rotating the arrow, is instructive: they find that the arrow is rotated by the time they return to their starting point. This does not happen on a flat surface, and is an identifying characteristic of curvature.

What the travelling is doing is parallel transporting a vector around a closed loop.

The simplest model we can construct of this scenario is to pick a starting point, $\mathcal{P}$, and a vector $\vec{v}$ at that point. We then parallel transport it around a closed loop, returning to $\mathcal{P}$, and compare the vector we end up with to the one we started with. We will be comparing vectors at the same point, so there is no ambiguity about the comparison. To define a loop, we can build a parallelogram, which means we just need two small vectors that each serve as two opposite sides of that parallelogram (negated for the return journey).

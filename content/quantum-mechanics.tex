...
That is, we perform the conjugate transpose on the left, and now we have a row matrix that multiplies with the column matrix to produce a scalar.

The conjugate transpose serves here as a mapping from vectors to covectors: that is, it's working like a metric. As we've seen previously in working with real vectors spaces, if our basis vectors are orthonormal, our metric is the identity matrix, and we can essentially ignore the distinction between vectors and covectors: the dot product between two vectors will work just fine.

In QM it isn't quite that straightforward. Although we will always use an orthonormal basis to get our coordinate representation of a vector, the scalars are complex numbers. We have to take the complex conjugate of the coordinates to map vector coordinates to covector coordinates. Thus when calculating we have to be just as careful about distinguishing covectors from vectors as we would be if we couldn't assume orthonormality.

We can split the new inner product notation into separate pieces, so we can write $\langle a|$, known as a "bra", to mean the covector whose matrix representation in some orthonormal basis is a single row containing the complex conjugates of the elements in the single column of the matrix representing $|a \rangle$, known as a "ket". These names were whimsically chosen by Dirac so that $\langle a|b \rangle$ is a "bra-ket".

Thus it is valid to say that $\langle a|$ is the covector corresponding to $|a \rangle$, which is the same as saying that $\langle a|$ is a function that acts on some vector $|b \rangle$ to extract $|b \rangle$'s coordinate along basis vector $|a \rangle$, as in the expression $\langle a|b \rangle$. And in concrete matrix terms we can picture $\langle a|$ as a 1-row matrix (a row vector) that is the dual of the 1-column matrix (column vector) $|a \rangle$, and their corresponding coordinates are mutually complex conjugates.

And with this matrix representation in mind, it follows that we can write them the other way round from the inner product:

$$|b \rangle \langle \vec{a} |$$
...
$$|b \rangle \langle \vec{a} | \vec{c} \rangle$$
...
will produce a matrix with a single $1$ in the $n$th place of the diagonal. So if we sum over all $n$, we get the identity matrix, a matrix that makes no difference to whatever vector it applies to. Spelling this out, if our vector space has just two basis vectors:
...
\begin{itemize}
  ...
  \item despite which, there is an inner product that we can use to get a non-negative real number for the norm of a vector: $\sqrt{\langle a|a \rangle}$, and 
...
The real parameter of such a function is analogous to the integer index that labels the rows in a column vector; instead of fetching the $i$th coordinate by its position in the column, we evaluate the function with some real value $x$ to get its coordinate "at" $x$.

Similarly, whereas the inner product over discrete coordinates is:
...
To interpret the state vector physically, we choose a basis so we can resolve it into coordinates. Our choice of basis has to do with the observable quantity we are presently interested in, such as position, momentum, orientation or energy. If it may take on any real value, the state vector will have to be a function of that value; if it may only take on certain discrete values, it can be a column vector (albeit sometimes one with infinitely many rows) in which each row corresponds to one of those possible discrete values that the observable may exhibit when measured.

The information available from the state vector is, in general, probabilistic. Each coordinate, being a complex number, is related to the probability of the observable quantity taking on the value represented by that coordinate. The squared modulus of the coordinate (its value multiplied by the complex conjugate of its value) is the probability of obtaining that value, or if the state vector is a function $f(x)$, then:

$$\int_{a}^{b} f(x)^* f(x) \, dx$$

is the probability that $x$ will have a value somewhere between $a$ and $b$.

As a probability is a number between $0$ and $1$, it must be the case that the sum of the squared modulus of all the coordinates (or the above integral from $-\infty$ to $+\infty$) must be $1$. This is the same as saying that $\langle S | S \rangle = 1$ for any physically realistic state vector. Or to put it another way, the magnitude of a state vector is not significant, only the direction (i.e. the relative values of the coordinates in some basis). We will always fix the magnitude to be $1$.

Unsurprisingly, if one of the coordinates is $1$ and all the others are zero, the vector represents certainty that the observable has the value represented by that coordinate. But this also means that the state vector is equal to one of the basis vectors. Thus the basis vectors for an observable represent exact values that the observable may exhibit when measured. This way of thinking leads us to a problem when the state vector is a function of a continuous variable, because there is no such thing as a function that is zero everywhere except at a single point where it is $1$, such that the area under the function is $1$. That is a whole heap of mathematical nonsense.

Further, a measurement of the observable (or more precisely, any interaction producing subsequent behaviour that could be used to infer the value of the observable) causes the state vector to change to the basis vector of that observable corresponding to the measured value. This change is (at least in this theory) assumed to be instantaneous and to have no mechanism that we can deduce anything further about. And again, if we're dealing with a continuous variable, there is no such basis function for the state to snap into alignment with. The best we can expect is something like a very narrow spike around the measured value.

Thus after measuring an observable, subsequent measurements of the same observable will with certain produce the same result. And once again, this is not quite true in the continuous cases when the state vector is actually a function of a real variable. We don't expect to ever find such a system precisely aligned with a single base state, but instead to have at least some small spread of probabilities.

\section{Switching Basis}

Having constructed a column representation of a state vector in one basis, relating to one observable, we can switch to another. The operation for doing this will depend on both the "before" and "after" bases (§\ref{sec:vectors-change-basis}). A state vector contains everything knowable about a system, including all we can know about any of its observable quantities. By re-expressing the same state vector as a different set of coordinates in terms of the basis associated with a different observable, we recover the probability distribution for that observable.

The operation that switches basis will be a matrix if we're dealing with a finite-dimensional space, or something analogous to that if the space is infinite. As always when using a matrix to transform a vector's coordinates we need to be clear on whether we want to get a different vector in the same basis or the same vector in a different basis. In this case we're talking about the latter; a state vector represents something physically real, and we're just changing how we describe it. On the other hand the choice of basis is not entirely arbitrary because a basis relates to an observable quantity.
...
In QM operators associated with observables are Hermitian or self-adjoint\footnote{There is no consensus on the precise meanings of these terms. Physicists use them interchangeably for any operator on a complex Hilbert space, finite or not, whereas mathematicians reserve Hermitian for matrices acting on finite spaces. Meanwhile some are comfortable describing self-adjoint operators as \textit{symmetric} regardless of whether the space is real or complex, whereas some prefer to reserve that term for real spaces only.}, meaning that for an operator $\hat{O}$:

$$\langle a|\hat{O} \vec{b} \rangle = \langle \hat{O} a| \vec{b} \rangle$$
...
\begin{itemize}
  ...
  \item Regardless of representation, eigenvectors (§\ref{sec:vectors-eigen}) with distinct eigenvalues are orthogonal and complete (they span the space, so you can take a unit vector in each of these orthogonal directions and you have an orthonormal basis.)
...
The basis vectors of the observable are just unit vectors that are eigenvectors of the operator. That is, if you apply the observable's operator to every possible state vector, a subset of them will be scaled without any change to their alignment. There will be a set of orthogonal unit vectors that pass this "alignment preserving" test, and these form the basis of the observable.
...
\begin{itemize}
  ...
  \item Applying the operator to the state is not the same as changing its numerical coordinate representation to be in the basis associated with the observable for that operator.
\end{itemize}

If you apply the operator to a state vector (which is, as always, of unit magnitude) and it is already aligned with an eigenstate, then its alignment will not be affected and its magnitude will change to the experimental value you would get if you measured the observable in a system in that state. If it is not aligned with an eigenstate, the vector's alignment will change, but it will not snap into alignment with an eigenstate, which is what happens when a real world measurement is performed.
...
While we've discussed all this in terms of more easily pictured finite-dimensional vectors with discrete complex coordinates, all the same concepts translate to complex-valued functions of an integer or real parameter.
...
The complex plane should not be confused with vectors. Any given snapshot of $\Psi(x, t)$ at some instant $t$, given by a function $\psi(x)$, is itself an entire vector. The position $x$ labels a single infinitesimal coordinate of the vector, and every such coordinate is a complex number, which we can therefore visualise as a complex plane with an arrow on it.
...
If we integrate over a region $A$ between $x_1$ and $x_2$:
...
the result is \textit{proportional} to the probability of finding the electron in $A$.
...
That is, they are a function of position (we're still in the position basis, so the "coordinates" of the vector are labelled by positions), but there is a momentum variable $p$ in the definition. That value of $p$ determines which basis vector of momentum this $\phi(x)$ describes.
...
But now we're going to consider intrinsic angular momentum. This is introduced to explain how electrons are deflected in a magnetic field, behaviour which is classically suggestive of the electron being made of charged matter and spinning around an axis, although that physical interpretation seems impossible because the electron's radius (if it is non-zero) is extremely small. Antipodal points on the surface of the electron would have to be moving relative to each other faster than the speed of light. Pauli suggested glossing over this question and just accepting that electrons have an intrinsic angular momentum that cannot be interpreted in some comfortable classical way.

One shorthand for this is to simply say that an electron is a magnet. It therefore (at least in the simplest scenario) has something akin to an orientation, an arrow sticking out of it that points in some direction, giving it a magnetic moment in that direction. The way we discover the alignment of an electron is by sending it through a magnetic field. The electron's internal magnetic interacts with the field and the electron experiences a force.

We're going to simplify things by considering an idealised particle in place of the electron, because a real electron has an electric charge, and so constitutes an electric current which has its own magnetic interaction. Our idealised particle will just be a tiny magnet, and all its interaction with magnetic fields will be due only to its intrinsic magnetic moment.\footnote{A silver atom was used in the original experiments, and it is a close approximation to this idealised particle.}

The magnetic fields we're going to use will have the interesting property that, rather than being simply uniform, they will increase in strength along a direction we can choose. This gradient will cause any magnet of moment $\vec{\mu}$ passing through to experience a force:

$$
\vec{F} = \grad(\vec{\mu} \cdot \vec{B})
$$

The magnetic field vector $\vec{B}$ is a function of position, so the dot product here is a scalar field, and then we take the gradient of that, which gives us a vector field: the force felt by the magnet. The variation in the field will be in one direction, say the $z$-direction, so there is only one significant component to the force:
...
Clearly the force is aligned with $z$, and its strength only depends on the $z$-component of the particle's magnetic moment, which should, according to our classical assumptions, be a continuously varying quantity that can be positive or negative, meaning that the force could be of a range of strengths and may be directed either way along our chosen $z$-axis.

So a beam of randomly orientated particles fired into the magnetic field would be expected to fan out in a continuum of different directions.

But this is not what happens. Instead the beam splits into just two concentrated beams. It's as if every particle happened to be pre-aligned with the axis of our magnetic field's gradient, in one of the two directions along that alignment. And yet this can't be true, because we can rotate our magnetic field around the beam, and this causes the split output beams to rotate with it.

Instead we might posit that the particles somehow instantaneously snap into alignment with the field as they interact with it. Even this intuition is not going to hold up for long, but it's a start. Just don't get too enamoured of it -- in Quantum Mechanics, there always seems to be another experiment that overturns whatever comfortingly simple explanation you just came up with.

If we put another similar magnetic field in the path of one of the beams, with the exact same alignment, it's like we're repeating the same experiment, except this time there is only one output beam, implying that its input consists of particles all aligned the same way. All particles from the input beam are accounted for in the single output beam. This is consistent with our theory that the particles are snapping into alignment as they interact with the first field. If we slightly rotate the second magnetic field around its input beam, the other output beam appears, initially with very few particles.

Once we've rotated the field by 90\textdegree, both beams contain equal numbers of particles. So if the snapping theory is correct, the particle chooses which of two opposing directions to snap into based on the angle between its current direction and the gradient of the magnetic field, and we can gradually adjust the probabilities by gradually rotating the field.

The way we model the alignment state of an particle is with the simplest possible QM state space, having just two dimensions. Recall our fanciful state space for a system that can measured to be in exactly one of two moods, or else listening to exactly one of two musical genres. The moods were represented by state vectors $|a\rangle$ (affable) and $|b\rangle$ (bored), and the genres by state vectors $|c\rangle$ (country) and $|d\rangle$ (disco). 
...
\begin{figure}[h]
  \centering
    ...
    \end{tikzpicture}
  \centering
  \caption{Mood and genre} \label{fig:mood-and-genre-recap}
\end{figure}

So each observable has two special unit state vectors which are mutually orthogonal and which span the entire space, meaning that you can make any state vector by a linear combination of the two state vectors of one observable.

It follows that the genre state vectors can be expressed in terms of the mood state vectors. This means that a genre vector is absolutely not orthogonal to either mood vector, but is a blend of them, in fact a perfectly balanced blend:
...
We have to use the factor $1/\sqrt{2}$ to ensure that the genre vectors are unit vectors.

This picture gets us started on a representation for the particle's alignment state. First let's get our bearings in physical space, while not leaping to any conclusions about the quantum state space, which is very different.

Pick any direction in physical space and call it "up", and the opposite direction "down". These are the positive and negative directions along one dimension in physical space. But physical space is three dimensional: there's an orthogonal spatial dimension with a negative direction "left" and position direction "right", and a third orthogonal dimension with negative direction "back" and positive direction "forwards".

Each of the three dimensions is an observable with two possible values that can be determined experimentally, and we need to fit their corresponding state vectors into the quantum state space. $|u\rangle$ (up) and $|d\rangle$ (down), being distinct values of one observable, are like the mood state vectors $|a\rangle$ (affable) and $|b\rangle$ (bored). They must be represented by orthogonal state vectors. Think carefully about this: "up" and "down" in physical space point in exactly opposite directions, but their quantum state vectors do not; they are at right angles.

Hold a pencil pointing up to begin with, and then slowly rotate it to the right. Continue rotating until it's pointing straight down. Keep going round to the left and finally back to the starting position. Along its journey, the pencil visited these states, in this order: $|u\rangle$, $|r\rangle$, $|d\rangle$, $|l\rangle$.

\begin{figure}[h]    
  \caption{Rotating the pencil}
  \begin{subfigure}{0.5\textwidth}
      \centering
      \begin{tikzpicture}
        \draw (0,0) circle (1);
        \draw[dashed,->] (0,0) -- (0,1) node[anchor=south] {"up"};
        \draw[dashed,->] (0,0) -- (1,0) node[anchor=west] {"right"};
        \draw[dashed,->] (0,0) -- (0,-1) node[anchor=north] {"down"};
        \draw[dashed,->] (0,0) -- (-1,0) node[anchor=east] {"left"};
      \end{tikzpicture}
  \caption{Physical space} \label{fig:pencil-physical}
  \end{subfigure}
  \begin{subfigure}{0.5\textwidth}
      \centering
      \begin{tikzpicture}
        \draw (0,0) circle (1);
        \draw[dashed,->] (0,0) -- (-0.707,0.707) node[anchor=south east] {$|u\rangle$};
        \draw[dashed,->] (0,0) -- (0,1) node[anchor=south] {$|r\rangle$};
        \draw[dashed,->] (0,0) -- (0.707,0.707) node[anchor=south west] {$|d\rangle$};
        \draw[dashed,->] (0,0) -- (1,0) node[anchor=west] {$|l\rangle$};
        \draw[dashed,->] (0,0) -- (0.707,-0.707) node[anchor=north west] {$|u\rangle$ ?};
    \end{tikzpicture}
      \caption{Quantum space} \label{fig:pencil-quantum}
  \end{subfigure}
\end{figure}

Repeat the rotation and this time look at how the state vector rotates. A rotation by $\theta$ in physical space corresponds to a rotation by $\theta/2$ in the quantum state space.

The first alarming conclusion is that when the pencil gets back to the "up" state, the state vector is not where it started. In fact, it has rotated by 180\textdegree! This means that each physical direction the pencil visited corresponded to a unique \textit{alignment} in the quantum state space. The state vector can be thought of as pointing either way along that alignment.

The second issue is that we only covered two of the three dimensions of physical space and yet we've run out of places to put the state vectors for $|b\rangle$ (back) and $|f\rangle$ (forwards). The state vector $|b\rangle$ must relate to $|u\rangle$ in the same way that $|l\rangle$ does, because there's nothing special about any direction. The directions opposite to the existing state vectors already mean the same thing as those state vectors; only the alignment matters.

The solution is that the quantum state space is not real, but complex. Referring to our diagram, we can express $|l\rangle$ and $|r\rangle$ in the up/down basis like this:

$$
|l\rangle = \frac{1}{\sqrt{2}}(|d\rangle - |u\rangle)
$$
$$
|r\rangle = \frac{1}{\sqrt{2}}(|d\rangle + |u\rangle)
$$

We can express $|f\rangle$ and $|b\rangle$ in the same basis, but we need to use complex coefficients:

$$
|f\rangle = \frac{1}{\sqrt{2}}(|d\rangle + i|u\rangle)
$$
$$
|b\rangle = \frac{1}{\sqrt{2}}(|d\rangle - i|u\rangle)
$$

Our first insight, that opposite directions in state space correspond to the same physical direction, can now be broadened. We realised physically measurable states correspond to alignments, and then (thinking about a real vector space) we said that a given alignment has two directions. But in a complex vector space, scalars are complex. You can scale a vector by (for example) $i$ and this doesn't change its alignment either. The set of unitary complex numbers is infinite, forming the unit circle in the complex plane. So rather than just two directions per alignment, there's an infinite continuum of them. We can say that state vectors that differ by a complex factor of modulus $1$ correspond to the same measurable direction in physical space. So $|u\rangle$ and $-|u\rangle$ and $i|u\rangle$ and $-i|u\rangle$ all represent the same physical direction, "up". More generally $e^{i\theta}|u\rangle$ represents the same physical direction for any $\theta$.

It may be tempting to jump to the conclusion that vectors in a complex space are just like real vectors but with an extra single overall phase ingredient, one more degree of freedom (like twisting the pencil without reorientating it at all). But consider the sum:

$$
|f\rangle = \frac{1}{\sqrt{2}}(|d\rangle + i|u\rangle)
$$

There's only an imaginary factor in the $|u\rangle$ component. If you tried pulling out an overall phase factor $i$:

$$
|f\rangle = \frac{i}{\sqrt{2}}(-i|d\rangle + |u\rangle)
$$

the $i$ factor simply jumps to the $|d\rangle$ component. Does this mean that the resultant vector is "complex" in a way that doesn't apply to the ingredients $|u\rangle$ and $|d\rangle$? Not at all. If you expressed $|u\rangle$ in the front/back basis, a similar complex mix of phases would be required. It's not very different from a real vector space in which $\vec{a} = -\vec{b}$; the two vectors are colinear, pointing in opposite directions, and the way we've written it makes it look like $\vec{a}$ is the "negative" of $\vec{b}$, but it could equally well be written the other way round. Any vector may appear to have a mixture of negative, positive, imaginary or real ingredients depending on the basis we choose to express it in.

A two dimensional complex space requires four real numbers to describe a vector in components, because the complex components have a real part and an imaginary part (or a modulus and phase). So how does any of this match up with the three dimensions of physical space?

Grab that pencil again. Although we can find three orthogonal directions in physical space, we don't actually need three coordinates to describe the direction the pencil is pointing. The length of the pencil is constant and we hold it by the blunt end in a fixed spot. As we wave the pencil in any direction, the tip of the pencil moves around on the surface of a sphere whose radius is the length of the pencil. We only need two numbers to locate a point on a spherical surface (a $2$-sphere) embedded in 3-dimensional space, because the tip of the pencil is not moving freely throughout all of space. It's actually constrained to a 2-dimensional surface.

So in the quantum state space we have four real numbers to play with, but two of them are constrained by the twin facts that:

\begin{itemize}
  \item the length of state vectors is always normalised to $1$, and
  \item the overall unitary complex factor of a state vector doesn't change what physical direction it represents. 
\end{itemize}

This leaves us with two real numbers to carry the information about the direction of the pencil, which is exactly what we need.

So why not just use a 2-dimensional real vector space to represent the orientation of an particle? Although the overall unitary complex factor doesn't affect the physical direction, that doesn't mean it is entirely physically insignificant; as we consider more examples, we'll see that (just as with the position or momentum in one-dimension particle motion) there are actually physically detectable effects that depend on the state vector's overall phase. One very strange implication of this is that when you rotate the pencil all the way round to the starting orientation, it is now in a different quantum state (it contains a lot of particles that you just rotated!) To restore it to its original state, you need to execute a \textit{second} rotation all the way round.

Working in the up/down basis, the column vector representation for the two basis states are simply the two standard orthoronal unit vectors:

$$|u \rangle = \begin{bmatrix} 1 \\ 0 \end{bmatrix}$$

and:

$$|d \rangle = \begin{bmatrix} 0 \\ 1 \end{bmatrix}$$

When we measure the particle's alignment along the up/down axis, we get one or the other of those two directions, that is, the particle is "pointing" either in the postive or negative direction along the $z$-axis in physical space. It makes sense to say that the eigenvalues of the up/down observable are $+1$ and $-1$ (but bear in mind that we haven't said what units we're working in yet.)

The observable for this measurement along the $z$-axis is represented by the operator $\sigma_z$ that has $|u\rangle$ as an eigenvector with eigenvalue $+1$, and $|d\rangle$ as an eigenvector with eigenvalue $-1$. When operating on these two basis vectors, it performs a scaling by their corresponding eigenvalues:
...
The action of a Hermitian operator in a N-dimensional space is characterised entirely by N numbers that tell us how it affects the Nth eigenvector, and if you write the operator as a matrix \textit{in the basis aligned with the eigenvectors}, those N numbers will be the diagonal elements of the matrix, with all the other elements being zero.

So the matrix representation of $\sigma_z$ in the up/down basis is:
...
We could of course have represented the same operator in any other orthonormal basis. Transformations between such bases are unitary (preserving lengths and angles):

$$\hat{U} \hat{U}^\dagger = I$$

This means that such transformations in matrix form have the property that their inverse is equal to their conjugate transpose. Every possible orthonormal basis can be reached from any other by some unitary transformation $\hat{U}$, and then the conjugate transpose of that transformation $\hat{U}^\dagger$ will take you back to the original basis. So this gives us a way to find the matrix representation of a Hermitian operator in any basis, given its simple diagonal representation in the basis of its eigenvectors: we just "wrap" it in the appropriate unitary transformation and its conjugate transpose. The effect of this may be to put non-zero values in the off-diagonal elements, but preserving the self-adjointness (the fact that the matrix is equal to its own conjugate transpose).

If we do this for the $\sigma_z$ operator, we can generate the operators for the other two spatial dimensions, $\sigma_x$ and $\sigma_y$, which have the same eigenvalues but different eigenvectors.
...
This is a remarkable set of three matrices, in that they are all both Hermitian \textit{and} unitary. Furthermore there is nothing special about the directions they represent. The three spatial basis directions are just our choice of basis, and these three matrices correspond to those directions, and can be regarded as \textit{basis operators}. We can linearly combine them to get operators for any other direction in physical space.

Another important but very easy to grasp fact about Hermitian operators follows directly from the way they act by simply scaling along orthogonal directions, and how this means they can be represented by a diagonal matrix if we align our basis with those directions. It means that you can consider their action along each direction
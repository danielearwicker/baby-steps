...
Unsurprisingly, if one of the coordinates is $1$ and all the others are zero, the vector represents certainty that the observable has the value represented by that coordinate. But this also means that the state vector is equal to one of the basis vectors. Thus the basis vectors for an observable represent exact values that the observable may exhibit when measured.
...
One shorthand for this is to simply say that an electron is a magnet. It therefore has something akin to an orientation, an arrow sticking out of it that points in some direction, giving it a magnetic moment in that direction. The way we discover the alignment of an electron is by sending it through a uniform magnetic field. The electron's internal magnetic interacts with the field and the electron experiences a force.
...
The variation in the field will be in one direction, say the $z$-direction, so there is only one significant component to the force:
...
Clearly the force is aligned with $z$, and its strength only depends on the $z$-component of the particle's magnetic moment, which should be a continuously varying quantity that can be positive or negative, meaning that the force could be of a range of strengths and may be directed either way along our chosen $z$-axis.

Classically we'd expect the strength of the force to depend on the relative orientation of particles's internal magnet and the magnetic field direction. So a beam of randomly orientated particles fired into the magnetic field would be expected to fan out in a continuum of different directions.

But this is not what happens. Instead the beam splits into just two concentrated beams. It's as if every particle happened to be pre-aligned with the axis of our magnetic field's gradient. And yet this can't be true, because we can rotate our magnetic field around the beam, and this causes the split output beams to rotate with it.
...
The way we model the alignment state of an particle is with the simplest possible QM state space, having just two dimensions. Recall our fanciful state space for a system that can measured to be in exactly one of two moods, or else listening two exactly one of two musical genres. The moods were represented by state vectors $|a\rangle$ (affable) and $|b\rangle$ (bored), and the genres by state vectors $|c\rangle$ (country) and $|d\rangle$ (disco). 
